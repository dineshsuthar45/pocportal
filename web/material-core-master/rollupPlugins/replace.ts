// @ts-nocheck
import type { NullValue, Plugin, SourceDescription } from 'rollup';
import MagicString from 'magic-string';
import { createFilter } from '@rollup/pluginutils';
import { RollupReplaceOptions } from './replace.types';
...
export default function replace(options: RollupReplaceOptions = {}): Plugin {
	...
	const pattern = new RegExp(`${delimiters[0]}(${keys.join('|')})${delimiters[1]}${lookahead}`, 'g');

	return {
		name: 'replace',

		buildStart() {
			if (preventAssignment && ![true, false].includes(preventAssignment)) {
				this.warn({
					message:
		...
		renderChunk(code, chunk): SourceDescription | string | NullValue {
		...
		transform(code, id): SourceDescription | string | NullValue {
			if (!keys.length) return null;
			if (!filter(id)) return null;
			return executeReplacement(code, id);
		},
	...
	function executeReplacement(code, id): SourceDescription | string | NullValue {
		const magicString = new MagicString(code);
		if (!codeHasReplacements(code, id, magicString)) {
			return null;
		}

		const result: SourceDescription = { code: magicString.toString() };
	...
	function codeHasReplacements(code, id, magicString) {
		...
		const testPattern = new RegExp(`\x00`, 'g');
		while ((match = testPattern.exec(code))) {
			result = true;
